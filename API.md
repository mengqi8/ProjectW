...
api返回字段包含result、error，如果没有错误，则error为null，否则error为具体错误内容。返回结果保存在result中。
...
统计数据库中未处理、已处理、已取消的订单数量，另外本接口也返回了数据库中的所有的订单数量（即上述三种所有订单数量的和）。
...
### 3.4 按天统计订单量接口（所有状态的订单）

本接口返回从第一个有订单的日期，到最后一个有订单的日期之间的所有数据，包括日期和当日的订单数量，数据来源为 orderdetail 表。如果某天的订单量为0，则结果中不包含此条数据。
